...
- [x] Admin + support tooling
  ...
  - dead-letter API + dashboard now support server-side page/filter query params (`page`, `orderId`, `failedReason`) so triage can navigate beyond a single client-side window
...
- [ ] Admin usability pass (`apps/web/app/admin/*`, `apps/web/app/globals.css`)
  ...
  - sixth pass shipped: true mobile card fallback now covers `provider-task-triage`, completing mobile card coverage across the high-frequency admin list views
